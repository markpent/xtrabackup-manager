			...
			try {
				...
				try {
					...
					$xbCommand = 'ssh -o StrictHostKeyChecking=no -p '.$hostInfo['ssh_port'].' '.$sbInfo['backup_user'].'@'.$hostInfo['hostname'].
								" 'cd $tempDir ; innobackupex ".$sbInfo['extra_innobackupex_args']." --ibbackup=".$xbBinary." --stream=xbstream ".$sbInfo['datadir_path']." --user=".$sbInfo['mysql_user'].
			...
			****************************/
			...
			try {
				...
				try {
					...
					$xbCommand = "ssh -o StrictHostKeyChecking=no -p ".$hostInfo['ssh_port']." ".$sbInfo['backup_user']."@".$hostInfo['hostname'].
								" 'cd $tempDir ; innobackupex ".$sbInfo['extra_innobackupex_args']." --ibbackup=".$xbBinary." --slave-info --incremental-lsn=".$lsn." ".$tempDir."/deltas".
